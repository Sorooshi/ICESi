...
def compute_contribution(s, p, rho, xi, indices):
    ...
    dj_p = np.sum(np.power(np.mean(p[indices, :], axis=0), 2))  # summary similarities
    contribution = rho * cv_y * len(indices) + xi * dj_p * len(indices)
...
if __name__ == '__main__':
    ...
    if run == 1:
        ...
        if setting_ != 'all':
            with open(os.path.join('../data', "SFNF_" + name + "-" + pp + "-" + setting_ + "-" +
                                              str(feature_embedding_is_inner) + ".pickle"), 'wb') as fp:
                pickle.dump(out_ms, fp)

        if setting_ == 'all':
            with open(os.path.join('../data', "SFNF_" + name + "-" + pp + "-" +
                                              str(feature_embedding_is_inner) + ".pickle"), 'wb') as fp:
                pickle.dump(out_ms, fp)
