...
def stage2(s, p, output_stage1, k, rho, xi):
    ...
    ss_s = np.zeros([r, 1])  # Summary Similarity of S
    ...
    for i in range(n):
        ...
        if i in remained_indices:  # and not (i in p_abnormalities):
            ...
            ss_s[index] = np.sum(s[i, pivot] + s[pivot, i] + s[i, i] + s[pivot, pivot])
            lambda_s = np.divide(ss_s[index], 2 ** 2)

            ss_p[index] = np.sum(p[i, pivot] + p[pivot, i] + p[i, i] + p[pivot, pivot])
            lambda_p = np.divide(ss_p[index], 2 ** 2)

            cntr[index, :] = (rho*ss_s[index]*lambda_s + xi*ss_p[index]*lambda_p, i)
            index += 1
...
def stage3(s, p, output_stage2, k, rho, xi):
    ...
    ss_p = np.sum([p[i, j] for i in clustered_indices_k for j in clustered_indices_k])
    lambda_p = np.divide(ss_p, len(clustered_indices_k) ** 2)

    ss_s = np.sum([s[i, j] for i in clustered_indices_k for j in clustered_indices_k])
    lambda_s = np.divide(ss_s, len(clustered_indices_k) ** 2)

    cntr_old = rho * ss_s * lambda_s + xi * ss_p * lambda_p
    ...
    while f_2 and len(remained_indices) != 0:
        ...
        cntr = [rho * (np.sum([s[ii, jj] for ii in indices for jj in indices]) *
                       np.divide(np.sum([s[ii, jj] for ii in indices for jj in indices]), len(indices) ** 2)) +
                xi * (np.sum([p[ii, jj] for ii in indices for jj in indices]) *
    ...
    if num_elements == 2:  # if I'm not mistaken it should be only two elements not three!
        ...
        if len(remained_indices) > 3:
            ...
            cntr_org = rho * (np.sum([s[ii, jj] for ii in clustered_indices_k for jj in clustered_indices_k])
                                * np.divide(np.sum([s[ii, jj] for ii in clustered_indices_k for jj in
                                                    clustered_indices_k]), len(clustered_indices_k) ** 2)) + \
                       xi * (np.sum([p[ii, jj] for ii in clustered_indices_k for jj in clustered_indices_k])
            ...
            # without pivot
            cntr_norg = rho * (np.sum([s[ii, jj] for ii in clustered_indices_k for jj in clustered_indices_k])
                               * np.divide(np.sum([s[ii, jj] for ii in clustered_indices_k for jj in
                                                   clustered_indices_k]), len(clustered_indices_k) ** 2)) + \
                        xi * (np.sum([p[ii, jj] for ii in clustered_indices_k for jj in clustered_indices_k])
                              * np.divide(np.sum([p[ii, jj] for ii in clustered_indices_k for jj in
...
def run_ANomalous_Cluster(pivot, s, p, rho, xi):
    ...
    while f_1:
        ...
        output_stage2 = stage2(s=s, p=p, output_stage1=output_stage1, k=k, rho=rho, xi=xi)
        output_stage3 = stage3(s=s, p=p, output_stage2=output_stage2, k=k, rho=rho, xi=xi)
...
if __name__ == '__main__':
    ...
    if run == 1:
                    ...
                    if str(setting) == setting_:
                        ...
                        for repeat, matrices in repeats.items():
                            ...
                            if data_type == "NP".lower() and with_noise == 0:
                                s_inn, s_aff = sg.generate_similarities(y=y_rng)
                                if feature_embedding_is_inner == 1:
                                    s, s_sum_sim, s_ave_sim, s_u, s_u_sum_sim, s_u_ave_sim, s_m, s_m_sum_sim, \
                                        s_m_ave_sim, s_l, s_l_sum_sim, s_l_ave_sim = pt.preprocess_p(p=s_inn)
                                else:
                                    s, s_sum_sim, s_ave_sim, s_u, s_u_sum_sim, s_u_ave_sim, s_m, s_m_sum_sim, \
                                        s_m_ave_sim, s_l, s_l_sum_sim, s_l_ave_sim = pt.preprocess_p(p=s_aff)

                                tmp_ms = run_ANomalous_Cluster(pivot=pivot, s=s, p=p, rho=rho, xi=xi)

                            elif data_type == "Nz-u".lower() and with_noise == 0:
                                print("Nz-u")
                                ...
                                tmp_ms = run_ANomalous_Cluster(pivot=pivot, s=s, p=p_u, rho=rho, xi=xi)

                            elif data_type == "Nz-m".lower() and with_noise == 0:
                                print("Nz-m")
                                s_inn, s_aff = sg.generate_similarities(y=y_z)
                                ...
                                tmp_ms = run_ANomalous_Cluster(pivot=pivot, s=s, p=p_m, rho=rho, xi=xi)

                            elif data_type == "Nr-u".lower() and with_noise == 0:
                                print("Nr-u")
                                s_inn, s_aff = sg.generate_similarities(y=y_rng)
                            ...
                            elif data_type == "Nr-m".lower() and with_noise == 0:
                                print("Nr-m")
                                ...
                                tmp_ms = run_ANomalous_Cluster(pivot=pivot, s=s_u, p=p_u, rho=rho, xi=xi)
                                ...
                                tmp_ms = run_ANomalous_Cluster(pivot=pivot, s=s_m, p=p_m, rho=rho, xi=xi)
                                ...
                                tmp_ms = run_ANomalous_Cluster(pivot=pivot, s=s_l, p=p_l, rho=rho, xi=xi)
                            ...
                            if data_type == "NP".lower() and with_noise == 1:
                                ...
                                s_n_inn, s_n_aff = sg.generate_similarities(y=y_n_rng)

                                if feature_embedding_is_inner == 1:
                                    s_n, s_n_sum_sim, s_n_ave_sim, s_n_u, s_n_u_sum_sim, s_n_u_ave_sim, \
                                        s_n_m, s_n_m_sum_sim, s_n_m_ave_sim, s_n_l, s_n_l_sum_sim, \
                                        s_n_l_ave_sim = pt.preprocess_p(p=s_n_inn)
                                else:
                                    s_n, s_n_sum_sim, s_n_ave_sim, s_n_u, s_n_u_sum_sim, s_n_u_ave_sim, \
                                        s_n_m, s_n_m_sum_sim, s_n_m_ave_sim, s_n_l, s_n_l_sum_sim, \
                                        s_n_l_ave_sim = pt.preprocess_p(p=s_n_aff)

                                tmp_ms = run_ANomalous_Cluster(pivot=pivot, s=s_n, p=p, rho=rho, xi=xi)

                            elif data_type == "Nz-u".lower() and with_noise == 1:
                                print("Nz-u")
                                s_inn, s_aff = sg.generate_similarities(y=y_n_z)
                            ...
                            elif data_type == "Nz-m".lower() and with_noise == 1:
                                print("Nz-m")
                                s_inn, s_aff = sg.generate_similarities(y=y_n_z)
                            ...
                            elif data_type == "Nr-u".lower() and with_noise == 1:
                                print("Nr-u")
                                s_inn, s_aff = sg.generate_similarities(y=y_n_rng)
                            ...
                            elif data_type == "Nr-m".lower() and with_noise == 1:
                                print("Nr-m")
                                ...
                                s_n_inn, s_n_aff = sg.generate_similarities(y=y_n_z)
                                ...
                                tmp_ms = run_ANomalous_Cluster(pivot=pivot, s=s_n_u, p=p_u, rho=rho, xi=xi)
                                ...
                                tmp_ms = run_ANomalous_Cluster(pivot=pivot, s=s_n_m, p=p_m, rho=rho, xi=xi)
                                ...
                                tmp_ms = run_ANomalous_Cluster(pivot=pivot, s=s_n_l, p=p_l, rho=rho, xi=xi)
                ...
                for setting, repeats in DATA.items():
                    ...
                    for repeat, matrices in repeats.items():
                        ...
                        if data_type == "NP".lower() and with_noise == 0:
                            ...
                            tmp_ms = run_ANomalous_Cluster(pivot=pivot, s=s_u, p=p_rng, rho=rho, xi=xi)
                            ...
                            tmp_ms = run_ANomalous_Cluster(pivot=pivot, s=s_u, p=p_z, rho=rho, xi=xi)
        ...
        if setting_ != 'all':
            with open(os.path.join('../data', "SNN_" + name + "-" + pp + "-" + setting_ + "-" +
                                              str(feature_embedding_is_inner) + ".pickle"), 'wb') as fp:
                pickle.dump(out_ms, fp)

        if setting_ == 'all':
            with open(os.path.join('../data', "SNN_" + name + "-" + pp + "-" +
                                              str(feature_embedding_is_inner) + ".pickle"), 'wb') as fp:
                pickle.dump(out_ms, fp)
        ...
        #         else:
        #             gt = DATA[setting][repeat]['GT']
        #         lp, _ = pt.flat_clustering_results(tmp_out_ms)
        #         ARI.append(metrics.adjusted_rand_score(gt, lp))
        #         NMI.append(metrics.adjusted_mutual_info_score(gt, lp))
        #
