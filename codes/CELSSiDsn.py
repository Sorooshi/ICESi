...
def compute_contribution(s, p, rho, xi, indices):
    ...
    ss_s = np.sum([s[i, j] for i in indices for j in indices])  # summary similarities of S (embedded Y)
    mu = np.divide(ss_s, len(indices) ** 2)
    dj_p = np.sum(np.power(np.mean(p[indices, :], axis=0), 2))  # mean of links in network space
    contribution = rho * mu * ss_s + xi * dj_p * len(indices)
...
if __name__ == '__main__':
    ...
    if run == 1:
        ...
        if setting_ != 'all':
            with open(os.path.join('../data', "SNNF_" + name + "-" + pp + "-" + setting_ + "-" +
                                              str(feature_embedding_is_inner) + ".pickle"), 'wb') as fp:
                pickle.dump(out_ms, fp)

        if setting_ == 'all':
            with open(os.path.join('../data', "SNNF_" + name + "-" + pp + "-" +
                                              str(feature_embedding_is_inner) + ".pickle"), 'wb') as fp:
                pickle.dump(out_ms, fp)
